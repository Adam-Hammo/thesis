...
void Report(void)
{
      ...
      static FILE **xfile, **ufile, **xffile, **uffile, **oxfile, **oufile;
      static FILE **pnfile, **vnfile, **wnfile, **qnfile;
      static FILE **ConstraintFile;
      static FILE *pn2file, *qn2file;
      ...
      if (First) {
         ...
         oxfile = (FILE **) calloc(Nsc,sizeof(FILE *));
         oufile = (FILE **) calloc(Nsc,sizeof(FILE *));
         pnfile = (FILE **) calloc(Nsc,sizeof(FILE *));
         vnfile = (FILE **) calloc(Nsc,sizeof(FILE *));
         wnfile = (FILE **) calloc(Nsc,sizeof(FILE *));
         qnfile = (FILE **) calloc(Nsc,sizeof(FILE *));
         ...
         for(Isc=0;Isc<Nsc;Isc++) {
            if (SC[Isc].Exists) {
               ...
               sprintf(s,"pn%02ld.42",Isc);
               pnfile[Isc] = FileOpen(InOutPath,s,"w");
               sprintf(s,"vn%02ld.42",Isc);
               vnfile[Isc] = FileOpen(InOutPath,s,"w");
               sprintf(s,"wn%02ld.42",Isc);
               wnfile[Isc] = FileOpen(InOutPath,s,"w");
               sprintf(s,"qn%02ld.42",Isc);
               qnfile[Isc] = FileOpen(InOutPath,s,"w");
               sprintf(s,"ox%02ld.42",Isc);
               oxfile[Isc] = FileOpen(InOutPath,s,"w");
               sprintf(s,"ou%02ld.42",Isc);
               oufile[Isc] = FileOpen(InOutPath,s,"w");
         ...
         sprintf(s,"pn12_2.42");
         pn2file = FileOpen(InOutPath,s,"w");
         sprintf(s,"qn12_2.42");
         qn2file = FileOpen(InOutPath,s,"w");
      ...
      if (OutFlag) {
         ...
         for(Isc=0;Isc<Nsc;Isc++) {
            if (SC[Isc].Exists) {
               ...
               fprintf(xfile[Isc],"\n");
               for(i=0;i<3;i++) fprintf(oxfile[Isc],"%18.12le ",O->PosN[i]);
               fprintf(oxfile[Isc],"\n");
               for(i=0;i<3;i++) fprintf(oufile[Isc],"%18.12le ",O->VelN[i]);
               fprintf(oufile[Isc],"\n");
               for(i=0;i<3;i++) fprintf(pnfile[Isc],"%18.12le ",S->PosN[i]);
               fprintf(pnfile[Isc],"\n");
               for(i=0;i<3;i++) fprintf(vnfile[Isc],"%18.12le ",S->VelN[i]);
               fprintf(vnfile[Isc],"\n");
               for(i=0;i<4;i++) fprintf(qnfile[Isc],"%18.12le ",B->qn[i]);
               fprintf(qnfile[Isc],"\n");
               for(i=0;i<3;i++) fprintf(wnfile[Isc],"%18.12le ",B->wn[i]);
               fprintf(wnfile[Isc],"\n");
               if (SC[Isc].FlexActive) {
               ...
               if (Isc == 12) {
                  B = &S->B[1];
                  for(i=0;i<3;i++) fprintf(pn2file,"%18.12le ",B->pn[i]);
                  fprintf(pn2file,"\n");
                  for(i=0;i<4;i++) fprintf(qn2file,"%18.12le ",B->qn[i]);
                  fprintf(qn2file,"\n");
               }
            }
         }

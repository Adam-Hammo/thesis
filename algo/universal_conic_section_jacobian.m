...
r = a_semi*(1 - (1 - (norm(init_pos)/a_semi))*cos(phi)) + ...
    init_pos'*init_vel*sqrt(a_semi/mu)*sin(phi);
...
phi11 = (norm(pos)/mu)*(vel - init_vel)*(vel - init_vel)' + ...
        (norm(init_pos)^(-3))*(norm(init_pos)*(1 - f)*pos*init_pos' + ...
         c*vel*init_pos') + f*eye(3);
     
phi12 = (norm(init_pos)/mu)*(1 - f)*((pos - init_pos)*init_vel' - ...
    (vel - init_vel)*init_pos') + (c/mu)*vel*init_vel' + g*eye(3);

phi21 = -(norm(init_pos)^(-2))*(vel - init_vel)*init_pos' - ...
        (norm(pos)^(-2))*pos*(vel - init_vel)' - ...
        (mu*c/((norm(pos)^3)*(norm(init_pos)^3)))*pos*init_pos' ...
        + fdot*(eye(3) - (norm(init_pos)^(-2))*pos*pos' + ...
        (1/(mu*norm(pos)))*(pos*vel' - vel*pos')*pos*(vel - init_vel)');
    
phi22 = (norm(init_pos)/mu)*(vel - init_vel)*(vel - init_vel)' + ...
        (norm(init_pos)^(-3))*(norm(init_pos)*(1 - f)*pos*init_pos' - ...
        c*pos*vel') + gdot*eye(3);

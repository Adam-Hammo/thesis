  ...
  methods
    function obj = ISAM2ThresholdMapValue(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(2698, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'char') && isa(varargin{2},'double') && size(varargin{2},2)==1
        my_ptr = gtsam_wrapper(2699, varargin{1}, varargin{2});
      elseif nargin == 1 && isa(varargin{1},'gtsam.ISAM2ThresholdMapValue')
        my_ptr = gtsam_wrapper(2700, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2701, obj.ptr_gtsamISAM2ThresholdMapValue);
    end

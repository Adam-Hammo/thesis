...
classdef RangeFactorWithTransform2D < gtsam.NoiseModelFactor
  ...
  methods
    function obj = RangeFactorWithTransform2D(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3445, varargin{2});
        end
        base_ptr = gtsam_wrapper(3444, my_ptr);
      elseif nargin == 5 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'double') && isa(varargin{4},'gtsam.noiseModel.Base') && isa(varargin{5},'gtsam.Pose2')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3446, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3447, obj.ptr_gtsamRangeFactorWithTransform2D);
    end
    ...
    function varargout = measured(this, varargin)
      ...
      error('Arguments do not match any overload of function gtsam.RangeFactorWithTransform2D.measured');
    end

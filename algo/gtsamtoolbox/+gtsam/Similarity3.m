...
%
%-------Methods-------
%equals(Similarity3 sim, double tol) : returns bool
  ...
  methods
    function obj = Similarity3(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(1344, my_ptr);
      elseif nargin == 0
        my_ptr = gtsam_wrapper(1345);
      elseif nargin == 1 && isa(varargin{1},'double')
        my_ptr = gtsam_wrapper(1346, varargin{1});
      elseif nargin == 3 && isa(varargin{1},'gtsam.Rot3') && isa(varargin{2},'double') && size(varargin{2},1)==3 && size(varargin{2},2)==1 && isa(varargin{3},'double')
        my_ptr = gtsam_wrapper(1347, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 3 && isa(varargin{1},'double') && isa(varargin{2},'double') && size(varargin{2},2)==1 && isa(varargin{3},'double')
        my_ptr = gtsam_wrapper(1348, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 1 && isa(varargin{1},'double')
        my_ptr = gtsam_wrapper(1349, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1350, obj.ptr_gtsamSimilarity3);
    end
    ...
    function varargout = equals(this, varargin)
      % EQUALS usage: equals(Similarity3 sim, double tol) : returns bool
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 2 && isa(varargin{1},'gtsam.Similarity3') && isa(varargin{2},'double')
        varargout{1} = gtsam_wrapper(1351, this, varargin{:});
        return
      end
      error('Arguments do not match any overload of function gtsam.Similarity3.equals');
    end
  ...
  methods(Static = true)
    function varargout = Align(varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'gtsam.Point3Pairs')
        varargout{1} = gtsam_wrapper(1358, varargin{:});
        return
      end
      ...
      if length(varargin) == 1 && isa(varargin{1},'gtsam.Pose3Pairs')
        varargout{1} = gtsam_wrapper(1359, varargin{:});
        return
      end

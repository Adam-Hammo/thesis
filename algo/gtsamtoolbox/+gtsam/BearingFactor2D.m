...
classdef BearingFactor2D < gtsam.NoiseModelFactor
  ...
  methods
    function obj = BearingFactor2D(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3473, varargin{2});
        end
        base_ptr = gtsam_wrapper(3472, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'gtsam.Rot2') && isa(varargin{4},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3474, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3475, obj.ptr_gtsamBearingFactor2D);
    end
    ...
    function varargout = measured(this, varargin)
      ...
      error('Arguments do not match any overload of function gtsam.BearingFactor2D.measured');
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(3478, varargin{:});
      else

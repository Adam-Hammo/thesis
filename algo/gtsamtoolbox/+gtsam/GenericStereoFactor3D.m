...
classdef GenericStereoFactor3D < gtsam.NoiseModelFactor
  ...
  methods
    function obj = GenericStereoFactor3D(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3862, varargin{2});
        end
        base_ptr = gtsam_wrapper(3861, my_ptr);
      elseif nargin == 5 && isa(varargin{1},'gtsam.StereoPoint2') && isa(varargin{2},'gtsam.noiseModel.Base') && isa(varargin{3},'numeric') && isa(varargin{4},'numeric') && isa(varargin{5},'gtsam.Cal3_S2Stereo')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3863, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3864, obj.ptr_gtsamGenericStereoFactor3D);
    end
    ...
    function varargout = calibration(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3865, this, varargin{:});
        return
      end
    ...
    function varargout = measured(this, varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3866, this, varargin{:});
        return
      end
    ...
    function varargout = string_serialize(this, varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3867, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(3868, varargin{:});
      else

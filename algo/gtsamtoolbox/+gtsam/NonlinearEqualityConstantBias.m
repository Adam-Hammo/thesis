  ...
  methods
    function obj = NonlinearEqualityConstantBias(varargin)
      ...
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.imuBias.ConstantBias')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3091, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.imuBias.ConstantBias') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3092, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3093, obj.ptr_gtsamNonlinearEqualityConstantBias);
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(3095, varargin{:});
      else

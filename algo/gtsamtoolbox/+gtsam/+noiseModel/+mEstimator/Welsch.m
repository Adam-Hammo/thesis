...
classdef Welsch < gtsam.noiseModel.mEstimator.Base
  ...
  methods
    function obj = Welsch(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1882, varargin{2});
        end
        base_ptr = gtsam_wrapper(1881, my_ptr);
      elseif nargin == 1 && isa(varargin{1},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1883, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1884, obj.ptr_gtsamnoiseModelmEstimatorWelsch);
    end
  ...
  methods(Static = true)
    function varargout = Create(varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(1887, varargin{:});
        return
      end

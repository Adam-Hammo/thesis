...
classdef Fair < gtsam.noiseModel.mEstimator.Base
  ...
  methods
    function obj = Fair(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1854, varargin{2});
        end
        base_ptr = gtsam_wrapper(1853, my_ptr);
      elseif nargin == 1 && isa(varargin{1},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1855, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1856, obj.ptr_gtsamnoiseModelmEstimatorFair);
    end
  ...
  methods(Static = true)
    function varargout = Create(varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(1859, varargin{:});
        return
      end

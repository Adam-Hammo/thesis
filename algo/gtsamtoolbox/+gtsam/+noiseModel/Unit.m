...
classdef Unit < gtsam.noiseModel.Isotropic
  ...
  methods
    function obj = Unit(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1838, varargin{2});
        end
        base_ptr = gtsam_wrapper(1837, my_ptr);
      else
    ...
    function delete(obj)
      gtsam_wrapper(1839, obj.ptr_gtsamnoiseModelUnit);
    end
  ...
  methods(Static = true)
    function varargout = Create(varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'numeric')
        varargout{1} = gtsam_wrapper(1840, varargin{:});
        return
      end

...
classdef RangeFactor2D < gtsam.NoiseModelFactor
  ...
  methods
    function obj = RangeFactor2D(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3389, varargin{2});
        end
        base_ptr = gtsam_wrapper(3388, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'double') && isa(varargin{4},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3390, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3391, obj.ptr_gtsamRangeFactor2D);
    end
    ...
    function varargout = measured(this, varargin)
      ...
      error('Arguments do not match any overload of function gtsam.RangeFactor2D.measured');
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(3394, varargin{:});
      else

...
classdef NonlinearEquality2PinholeCameraCal3Bundler < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEquality2PinholeCameraCal3Bundler(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3175, varargin{2});
        end
        base_ptr = gtsam_wrapper(3174, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3176, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 2 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3177, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3178, obj.ptr_gtsamNonlinearEquality2PinholeCameraCal3Bundler);
    end

...
classdef GPSFactor < gtsam.NonlinearFactor
  ...
  methods
    function obj = GPSFactor(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(4290, varargin{2});
        end
        base_ptr = gtsam_wrapper(4289, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'double') && size(varargin{2},1)==3 && size(varargin{2},2)==1 && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(4291, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(4292, obj.ptr_gtsamGPSFactor);
    end
    ...
    function varargout = equals(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'gtsam.GPSFactor') && isa(varargin{2},'double')
        varargout{1} = gtsam_wrapper(4293, this, varargin{:});
        return
      end
    ...
    function varargout = measurementIn(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(4294, this, varargin{:});
        return
      end
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'char') && isa(varargin{2},'gtsam.KeyFormatter')
        gtsam_wrapper(4295, this, varargin{:});
        return
      end
      % PRINT usage: print(string s) : returns void
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(4296, this, varargin{:});
        return
      end
      % PRINT usage: print() : returns void
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        gtsam_wrapper(4297, this, varargin{:});
        return
      end

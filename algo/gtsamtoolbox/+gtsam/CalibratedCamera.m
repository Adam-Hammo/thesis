...
%backproject(Point2 p, double depth, Ref<MatrixXd> Dresult_dpose, Ref<MatrixXd> Dresult_dp, Ref<MatrixXd> Dresult_ddepth) : returns Point3
%dim() : returns size_t
...
%project(Point3 point, Ref<MatrixXd> Dcamera, Ref<MatrixXd> Dpoint) : returns Point2
...
%range(Pose3 point, Ref<MatrixXd> Dcamera, Ref<MatrixXd> Dpose) : returns double
...
classdef CalibratedCamera < handle
  ...
  methods
    function obj = CalibratedCamera(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(1302, my_ptr);
      elseif nargin == 0
        my_ptr = gtsam_wrapper(1303);
      elseif nargin == 1 && isa(varargin{1},'gtsam.Pose3')
        my_ptr = gtsam_wrapper(1304, varargin{1});
      elseif nargin == 1 && isa(varargin{1},'double') && size(varargin{1},2)==1
        my_ptr = gtsam_wrapper(1305, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1306, obj.ptr_gtsamCalibratedCamera);
    end
    ...
    function varargout = backproject(this, varargin)
      ...
      % BACKPROJECT usage: backproject(Point2 p, double depth, Ref<MatrixXd> Dresult_dpose, Ref<MatrixXd> Dresult_dp, Ref<MatrixXd> Dresult_ddepth) : returns Point3
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 5 && isa(varargin{1},'double') && size(varargin{1},1)==2 && size(varargin{1},2)==1 && isa(varargin{2},'double') && isa(varargin{3},'Eigen.RefMatrixXd') && isa(varargin{4},'Eigen.RefMatrixXd') && isa(varargin{5},'Eigen.RefMatrixXd')
        varargout{1} = gtsam_wrapper(1308, this, varargin{:});
        return
      end
      error('Arguments do not match any overload of function gtsam.CalibratedCamera.backproject');
    end
    ...
    function varargout = pose(this, varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(1312, this, varargin{:});
        return
      end
    ...
    function varargout = project(this, varargin)
      ...
      % PROJECT usage: project(Point3 point, Ref<MatrixXd> Dcamera, Ref<MatrixXd> Dpoint) : returns Point2
      % Doxygen can be found at https://gtsam.org/doxygen/
    ...
    function varargout = range(this, varargin)
      ...
      % RANGE usage: range(Pose3 point, Ref<MatrixXd> Dcamera, Ref<MatrixXd> Dpose) : returns double
      % Doxygen can be found at https://gtsam.org/doxygen/
  ...
  methods(Static = true)
    function varargout = Dim(varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(1324, varargin{:});
        return
      end
    ...
    function varargout = Level(varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'gtsam.Pose2') && isa(varargin{2},'double')
        varargout{1} = gtsam_wrapper(1325, varargin{:});
        return
      end
    ...
    function varargout = Project(varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'double') && size(varargin{1},1)==3 && size(varargin{1},2)==1
        varargout{1} = gtsam_wrapper(1326, varargin{:});
        return
      end
    ...
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(1327, varargin{:});
      else

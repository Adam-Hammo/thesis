...
classdef NonlinearEquality2PinholeCameraCal3Unified < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEquality2PinholeCameraCal3Unified(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3187, varargin{2});
        end
        base_ptr = gtsam_wrapper(3186, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3188, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 2 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3189, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3190, obj.ptr_gtsamNonlinearEquality2PinholeCameraCal3Unified);
    end

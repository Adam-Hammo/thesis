...
classdef NonlinearEquality2Pose3 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEquality2Pose3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3151, varargin{2});
        end
        base_ptr = gtsam_wrapper(3150, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3152, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 2 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3153, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3154, obj.ptr_gtsamNonlinearEquality2Pose3);
    end

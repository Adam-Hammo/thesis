...
classdef NonlinearEqualitySO3 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualitySO3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3006, varargin{2});
        end
        base_ptr = gtsam_wrapper(3005, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.SO3')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3007, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.SO3') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3008, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3009, obj.ptr_gtsamNonlinearEqualitySO3);
    end
    ...
    function varargout = string_serialize(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3010, this, varargin{:});
      else

...
classdef PoseRotationPrior2D < gtsam.NoiseModelFactor
  ...
  methods
    function obj = PoseRotationPrior2D(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3884, varargin{2});
        end
        base_ptr = gtsam_wrapper(3883, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Pose2') && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3885, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3886, obj.ptr_gtsamPoseRotationPrior2D);
    end
    ...
    function varargout = measured(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3887, this, varargin{:});
        return
      end

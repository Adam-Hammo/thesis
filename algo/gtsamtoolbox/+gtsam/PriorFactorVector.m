...
classdef PriorFactorVector < gtsam.NoiseModelFactor
  ...
  methods
    function obj = PriorFactorVector(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2817, varargin{2});
        end
        base_ptr = gtsam_wrapper(2816, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'double') && size(varargin{2},2)==1 && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2818, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2819, obj.ptr_gtsamPriorFactorVector);
    end
    ...
    function varargout = prior(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(2820, this, varargin{:});
        return
      end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(2822, varargin{:});
      else

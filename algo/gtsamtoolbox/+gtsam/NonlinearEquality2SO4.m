...
classdef NonlinearEquality2SO4 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEquality2SO4(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3127, varargin{2});
        end
        base_ptr = gtsam_wrapper(3126, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3128, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 2 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3129, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3130, obj.ptr_gtsamNonlinearEquality2SO4);
    end

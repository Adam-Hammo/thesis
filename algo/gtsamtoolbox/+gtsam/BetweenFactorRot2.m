...
classdef BetweenFactorRot2 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = BetweenFactorRot2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3550, varargin{2});
        end
        base_ptr = gtsam_wrapper(3549, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'gtsam.Rot2') && isa(varargin{4},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3551, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3552, obj.ptr_gtsamBetweenFactorRot2);
    end
    ...
    function varargout = measured(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3553, this, varargin{:});
        return
      end
    ...
    function varargout = string_serialize(this, varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3554, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(3555, varargin{:});
      else

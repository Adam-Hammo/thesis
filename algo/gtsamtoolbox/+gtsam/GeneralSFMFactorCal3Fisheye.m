...
classdef GeneralSFMFactorCal3Fisheye < gtsam.NoiseModelFactor
  ...
  methods
    function obj = GeneralSFMFactorCal3Fisheye(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3817, varargin{2});
        end
        base_ptr = gtsam_wrapper(3816, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'double') && size(varargin{1},1)==2 && size(varargin{1},2)==1 && isa(varargin{2},'gtsam.noiseModel.Base') && isa(varargin{3},'numeric') && isa(varargin{4},'numeric')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3818, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3819, obj.ptr_gtsamGeneralSFMFactorCal3Fisheye);
    end
    ...
    function varargout = measured(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3820, this, varargin{:});
        return
      end

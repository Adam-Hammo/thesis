...
classdef SubgraphSolverParameters < gtsam.ConjugateGradientParameters
  ...
  methods
    function obj = SubgraphSolverParameters(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2253, varargin{2});
        end
        base_ptr = gtsam_wrapper(2252, my_ptr);
      elseif nargin == 0
        [ my_ptr, base_ptr ] = gtsam_wrapper(2254);
      else
    ...
    function delete(obj)
      gtsam_wrapper(2255, obj.ptr_gtsamSubgraphSolverParameters);
    end

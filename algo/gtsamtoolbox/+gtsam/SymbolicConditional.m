...
classdef SymbolicConditional < gtsam.SymbolicFactor
  ...
  methods
    function obj = SymbolicConditional(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3309, varargin{2});
        end
        base_ptr = gtsam_wrapper(3308, my_ptr);
      elseif nargin == 0
        [ my_ptr, base_ptr ] = gtsam_wrapper(3310);
      elseif nargin == 1 && isa(varargin{1},'gtsam.SymbolicConditional')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3311, varargin{1});
      elseif nargin == 1 && isa(varargin{1},'numeric')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3312, varargin{1});
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3313, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'numeric')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3314, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'numeric') && isa(varargin{4},'numeric')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3315, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3316, obj.ptr_gtsamSymbolicConditional);
    end
    ...
    function varargout = firstFrontalKey(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3318, this, varargin{:});
        return
      end
      error('Arguments do not match any overload of function gtsam.SymbolicConditional.firstFrontalKey');
    end
    ...
    function varargout = nrFrontals(this, varargin)
      ...
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3319, this, varargin{:});
        return
      end
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'char') && isa(varargin{2},'gtsam.KeyFormatter')
        gtsam_wrapper(3321, this, varargin{:});
        return
      end
      % PRINT usage: print(string s) : returns void
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(3322, this, varargin{:});
        return
      end
      % PRINT usage: print() : returns void
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        gtsam_wrapper(3323, this, varargin{:});
        return
      end

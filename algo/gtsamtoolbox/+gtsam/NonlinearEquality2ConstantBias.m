...
classdef NonlinearEquality2ConstantBias < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEquality2ConstantBias(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3193, varargin{2});
        end
        base_ptr = gtsam_wrapper(3192, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3194, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 2 && isa(varargin{1},'gtsam.Key') && isa(varargin{2},'gtsam.Key')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3195, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3196, obj.ptr_gtsamNonlinearEquality2ConstantBias);
    end

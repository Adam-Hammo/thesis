...
classdef PriorFactorRot3 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = PriorFactorRot3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2873, varargin{2});
        end
        base_ptr = gtsam_wrapper(2872, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Rot3') && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2874, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2875, obj.ptr_gtsamPriorFactorRot3);
    end
    ...
    function varargout = prior(this, varargin)
      ...
      error('Arguments do not match any overload of function gtsam.PriorFactorRot3.prior');
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(2878, varargin{:});
      else

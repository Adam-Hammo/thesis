...
classdef EssentialMatrixConstraint < gtsam.NoiseModelFactor
  ...
  methods
    function obj = EssentialMatrixConstraint(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(3660, varargin{2});
        end
        base_ptr = gtsam_wrapper(3659, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'gtsam.EssentialMatrix') && isa(varargin{4},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(3661, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_wrapper(3662, obj.ptr_gtsamEssentialMatrixConstraint);
    end
    ...
    function varargout = equals(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'gtsam.EssentialMatrixConstraint') && isa(varargin{2},'double')
        varargout{1} = gtsam_wrapper(3663, this, varargin{:});
        return
      end
    ...
    function varargout = evaluateError(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'gtsam.Pose3') && isa(varargin{2},'gtsam.Pose3')
        varargout{1} = gtsam_wrapper(3664, this, varargin{:});
        return
      end
    ...
    function varargout = measured(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_wrapper(3665, this, varargin{:});
        return
      end
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'char') && isa(varargin{2},'gtsam.KeyFormatter')
        gtsam_wrapper(3666, this, varargin{:});
        return
      end
      % PRINT usage: print(string s) : returns void
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(3667, this, varargin{:});
        return
      end
      % PRINT usage: print() : returns void
      % Doxygen can be found at https://gtsam.org/doxygen/
      if length(varargin) == 0
        gtsam_wrapper(3668, this, varargin{:});
        return
      end

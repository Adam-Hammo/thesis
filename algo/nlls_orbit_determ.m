function [r0,v0] = nlls_orbit_determ(obs,GS_ECEF,init_posvel_guess,time_last_vernal_equinox)
...
% See Week 5 Slides 19 to 31 for Details on non-linear least squares for
% orbit determination from ground station tracking measurements

% FILL IN HERE
% time_last_vernal_equinox = ....

[gs_lat, gs_long, gs_alt] = ecef_to_llh_geocentric(GS_ECEF);
...
while (norm(delta_x) > tol)
    ...
    for i = 1:num_obs
        ...
        ecef = eci_to_ecef_geocentric(pos',current_obs_time - time_last_vernal_equinox);
        lgcv = ecef_to_lgcv(ecef,gs_lat,gs_long,gs_alt);
        rae = cart_to_polar(lgcv);
        expected_obs = [rae(3) deg2rad(rae(1:2))];
    ...
    if mod(iter,200) == 0
        disp(iter + " iterations done. Delta x size " + norm(delta_x))
    end
